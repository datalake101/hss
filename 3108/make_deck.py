#!/usr/bin/env python3
"""
make_deck.py — Turn a structured lecture .txt into a premium PPTX deck.

Layout:  filled left panel (label + title + page number) | bulleted content right.
Usage:   python make_deck.py lecture1.txt [-o output.pptx] [-p crimson|midnight|forest|graphite|wine]
"""
...
def style_run(run, text, font, size, hexcolor, bold=False, tracking=None):
    ...
    if tracking:  # letterspacing, in 1/100 pt (premium small-caps look)
        run._r.get_or_add_rPr().set("spc", str(tracking))

def set_hanging_indent(paragraph, indent_in=0.38):
    """Clean hanging indent without triggering OpenXML schema validation issues."""
...
def fit_body_size(bullets, width_in, height_in, max_size=18.0, min_size=10.5):
    """Shrink font until estimated wrapped height fits the content area."""
    size = max_size
    while size > min_size:
        ...
        total += max(0, len(bullets) - 1) * size * spacing_ratio / 72          # spacing
        if total <= height_in:
            break
        size -= 0.5
    return size
    ...
    for i, s in enumerate(slides, start=1):
        ...
        if s["bullets"]:
            ...
            for j, bullet in enumerate(s["bullets"]):
                ...
                if m:  # typographic hierarchy: "Example:" / "1796:" / "Lesson:" ...
...
if __name__ == "__main__":
    is_ipykernel = any("ipykernel" in str(arg) for arg in sys.argv) or "-f" in sys.argv

    ap = argparse.ArgumentParser(description="Generate premium PPTX from structured lecture text.")
    ap.add_argument("input", nargs="?", default=None, help="lecture .txt file")
    ap.add_argument("-o", "--output", default=None, help="output .pptx path")
    ap.add_argument("-p", "--palette", default="crimson",
                    choices=list(PALETTES), help="color palette name")

    # If running inside Jupyter / IPython kernel launcher, ignore kernel CLI flags
    if is_ipykernel:
    ...
    # If no input file is specified, batch convert all found lecture files
    if not args.input:
        base_dir = os.path.dirname(os.path.abspath(__file__)) if "__file__" in globals() else os.getcwd()
        raw_candidates = (
            glob.glob(os.path.join(base_dir, "[lL]ecture*.txt"))
            + glob.glob("[lL]ecture*.txt")
        )
        # Deduplicate and sort
        candidates = sorted(list(dict.fromkeys([os.path.abspath(c) for c in raw_candidates if os.path.isfile(c)])))
        if candidates:
            print(f"🚀 Found {len(candidates)} lecture files. Generating decks...\n")
            for c in candidates:
                out = re.sub(r"\.txt$", "", c) + "_premium.pptx"
                title, label, slides = parse(c)
                build(title, label, slides, args.palette, out)
            print("\n🎉 All lecture decks generated successfully!")
        else:
            ap.print_help()
            sys.exit(1)
    else:
        out = args.output or re.sub(r"\.txt$", "", args.input) + "_premium.pptx"
        title, label, slides = parse(args.input)
        build(title, label, slides, args.palette, out)
